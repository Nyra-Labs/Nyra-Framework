...
build_cxx_extensions() {
  ...
  # Copy the output of nyra_packages to the nyra_packages/extension/xx/lib.
  local out="out/$OS/$CPU"
  for extension in $out/nyra_packages/extension/*; do
    ...
    mkdir -p $app_dir/nyra_packages/extension/$extension_name/lib
    cp -r $extension/lib/* $app_dir/nyra_packages/extension/$extension_name/lib
  done
}
...
install_python_requirements() {
  ...
  # traverse the nyra_packages/extension directory to find the requirements.txt
  if [[ -d "nyra_packages/extension" ]]; then
    for extension in nyra_packages/extension/*; do
  ...
  # traverse the nyra_packages/system directory to find the requirements.txt
  if [[ -d "nyra_packages/system" ]]; then
    for extension in nyra_packages/system/*; do
      if [[ -f "$extension/requirements.txt" ]]; then
        pip install -r $extension/requirements.txt
      fi
    done
...
build_go_app() {
  ...
  go run nyra_packages/system/nyra_runtime_go/tools/build/main.go --verbose
  if [[ $? -ne 0 ]]; then

...
class BedrockLLM:
    ...
    def __init__(self, config: BedrockLLMConfig, nyra_env: TenEnv):
        self.config = config
        self.nyra_env = nyra_env

        if config.access_key and config.secret_key:
            self.nyra_env.log_info(f"BedrockLLM initialized with access key: {config.access_key}")
    ...
    def get_converse_stream(self, messages):
        ...
        if self.config.prompt:
            bedrock_req_params["system"] = [{"text": self.config.prompt}]

        try:
            response = self.client.converse_stream(**bedrock_req_params)
            return response
        except Exception as e:
            raise RuntimeError(f"GetConverseStream failed, err: {e}") from e

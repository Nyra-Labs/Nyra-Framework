...
class AliGPDBClient:
    def __init__(self, nyra_env, access_key_id, access_key_secret, endpoint):
    ...
    async def __thread_routine(self):
        self.nyra_env.log_info("client __thread_routine start")
        ...
        while not self.stopEvent.is_set():
            if not self.tasks.empty():
                ...
                for task in done:
                    if task.exception():
                        self.nyra_env.log_error(f"task exception: {task.exception()}")
                        future.set_exception(task.exception())
            else:
                await asyncio.sleep(0.1)
        self.nyra_env.log_info("client __thread_routine end")

...
class BingSearchToolExtension(AsyncLLMToolBaseExtension):
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        ...
        self.config = await BingSearchToolConfig.create_async(nyra_env=nyra_env)

        if not self.config.api_key:
            nyra_env.log_info("API key is missing, exiting on_start")
            return
    ...
    async def run_tool(
        ...
        if name == TOOL_NAME:
            result = await self._do_search(nyra_env, args)
            # result = LLMCompletionContentItemText(text="I see something")
            return {"content": json.dumps(result)}

    async def _do_search(self, nyra_env: AsyncTenEnv, args: dict) -> Any:
        if "query" not in args:
            raise ValueError("Failed to get property")

        query = args["query"]
        snippets = []
        results = await self._bing_search_results(nyra_env, query, count=self.k)
        if len(results) == 0:
    ...
    async def _initialize_session(self, nyra_env: AsyncTenEnv):
        if self.session is None or self.session.closed:
            nyra_env.log_debug("Initializing new session")
            self.session = aiohttp.ClientSession()

    async def _bing_search_results(self, nyra_env: AsyncTenEnv, search_term: str, count: int) -> List[dict]:
        await self._initialize_session(nyra_env)

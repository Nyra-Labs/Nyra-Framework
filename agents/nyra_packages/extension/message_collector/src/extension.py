...
def _text_to_base64_chunks(_: TenEnv, text: str, msg_id: str) -> list:
    ...
    while current_position < total_length:
        ...
        # nyra_env.log_debug(f"chunk estimate guess: {count}")
...
class MessageCollectorExtension(Extension):
    ...
    def on_start(self, nyra_env: TenEnv) -> None:
        ...
        self.loop.create_task(self._process_queue(nyra_env))
    ...
    def on_data(self, nyra_env: TenEnv, data: Data) -> None:
        ...
        try:
            chunks = _text_to_base64_chunks(nyra_env, json.dumps(base_msg_data), message_id)
            for chunk in chunks:
                asyncio.run_coroutine_threadsafe(self._queue_message(chunk), self.loop)

        except Exception as e:
            nyra_env.log_warn(f"on_data new_data error: {e}")
            return
    ...
    async def _process_queue(self, nyra_env: TenEnv):
        while True:
            data = await self.queue.get()
            if data is None:
                break
            # process data
            nyra_data = Data.create("data")
            nyra_data.set_property_buf("data", data.encode())
            nyra_env.send_data(nyra_data)

...
class AsyncCozeExtension(AsyncLLMBaseExtension):
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        ...
        self.config = await CozeConfig.create_async(nyra_env=nyra_env)
        ...
        try:
            ...
            nyra_env.log_error(f"Failed to create conversation {e}")
    ...
    async def on_data_chat_completion(
        ...
        sentences = []
        self.nyra_env.log_info(f"messages: {messages}")
            ...
            try:
                ...
                async with session.post(url, json=params, headers=headers) as response:
                    async for line in response.content:
                        if line:
                            try:
                                self.nyra_env.log_info(f"line: {line}")
                                ...
                                if decoded_line:
                                    if decoded_line.startswith("data:"):
                                        ...
                                        self.nyra_env.log_info(f"event: {event}")
                                        if event == "done":
                                            break
                                    else:
                                ...
                                self.nyra_env.log_error(f"Failed to stream chat: {e}")
            except Exception as e:
                traceback.print_exc()
                self.nyra_env.log_error(f"Failed to stream chat: {e}")
            finally:

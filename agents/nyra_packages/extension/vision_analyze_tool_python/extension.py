...
from nyra_ai_base.const import CMD_CHAT_COMPLETION_CALL
...
class VisionAnalyzeToolExtension(AsyncLLMToolBaseExtension):
    ...
    async def on_data(self, nyra_env: AsyncTenEnv, data: Data) -> None:
        ...
        nyra_env.log_debug("on_data name {}".format(data_name))

    async def on_audio_frame(
        self, nyra_env: AsyncTenEnv, audio_frame: AudioFrame
    ) -> None:
        audio_frame_name = audio_frame.get_name()
        nyra_env.log_debug("on_audio_frame name {}".format(audio_frame_name))

    async def on_video_frame(
        self, nyra_env: AsyncTenEnv, video_frame: VideoFrame
    ) -> None:
        video_frame_name = video_frame.get_name()
        nyra_env.log_debug("on_video_frame name {}".format(video_frame_name))
    ...
    async def run_tool(
        ...
        if name == "get_vision_chat_completion":
            ...
            nyra_env.log_info("send_cmd {}".format(message))
            cmd_result: CmdResult = await nyra_env.send_cmd(cmd)

...
class AsyncGlueExtension(AsyncLLMBaseExtension):
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        ...
        self.config = await GlueConfig.create_async(nyra_env=nyra_env)
        ...
        if self.config.enable_storage:
            ...
            if result.get_status_code() == StatusCode.OK:
                try:
                    ...
                    nyra_env.log_error("Failed to handle retrieve result {e}")
            else:
    ...
    async def on_cmd(self, nyra_env: AsyncTenEnv, cmd: Cmd) -> None:
        ...
        if cmd_name == CMD_IN_FLUSH:
            await self.flush_input_items(nyra_env)
            ...
            if self.config.greeting and self.users_count == 1:
                self.send_text_output(nyra_env, self.config.greeting, True)
    ...
    async def on_call_chat_completion(
        self, nyra_env: AsyncTenEnv, **kargs: LLMCallCompletionArgs
    ) -> any:
        raise RuntimeError("Not implemented")

    async def on_data_chat_completion(
        self, nyra_env: AsyncTenEnv, **kargs: LLMDataCompletionArgs
    ) -> None:
        ...
        if not input_messages:
            nyra_env.log_warn("No message in data")
        else:
        ...
        async for message in response:
            self.nyra_env.log_debug(f"content: {message}")
            try:
                ...
                if c.usage:
                    self.nyra_env.log_info(f"usage: {c.usage}")
                    await self._update_usage(c.usage)
            except Exception as e:
                self.nyra_env.log_error(f"Failed to parse response: {message} {e}")
                traceback.print_exc()
        ...
        if calls:
            ...
            for _, call in calls.items():
                self.nyra_env.log_info(f"tool call: {call}")
            ...
            for r in responses:
                ...
                self.nyra_env.log_info(f"tool call response: {content} {r.id}")
                self.memory.put(
                    {
                        "role": "tool",
            ...
            await self.on_data_chat_completion(nyra_env)

        self.nyra_env.log_info(f"total_output: {total_output} {calls}")

    async def on_tools_update(
        self, nyra_env: AsyncTenEnv, tool: LLMToolMetadata
    ) -> None:
        # Implement the logic for tool updates
        return await super().on_tools_update(nyra_env, tool)
    ...
    async def handle_tool_call(self, call: ToolCall) -> ToolCallResponse:
        ...
        if result.get_status_code() == StatusCode.OK:
            ...
            self.nyra_env.log_info(f"tool_result: {call} {tool_result}")
    ...
    async def on_data(self, nyra_env: AsyncTenEnv, data: Data) -> None:
        ...
        nyra_env.log_info(f"on_data name {data_name}")
    ...
    async def _stream_chat(
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                self.nyra_env.log_info(f"payload before sending: {json.dumps(payload)}")
                headers = {
                    ...
                    async for line in response.content:
                        if line:
                            ...
                            if l.startswith("data:"):
                                ...
                                if content == "[DONE]":
                                    break
                                self.nyra_env.log_debug(f"content: {content}")
                                yield json.loads(content)
            except Exception as e:
                traceback.print_exc()
                self.nyra_env.log_error(f"Failed to handle {e}")
            finally:
                await session.close()
                session = None

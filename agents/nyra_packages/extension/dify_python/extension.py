...
from nyra_ai_base.types import LLMChatCompletionUserMessageParam, LLMDataCompletionArgs
...
class DifyExtension(AsyncLLMBaseExtension):
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        ...
        self.config = await DifyConfig.create_async(nyra_env=nyra_env)
        nyra_env.log_info(f"config: {self.config}")

        if not self.config.api_key:
            nyra_env.log_error("Missing required configuration")
            return
    ...
    async def on_data(self, nyra_env: AsyncTenEnv, data: Data) -> None:
        ...
        nyra_env.log_info("on_data name {}".format(data_name))
    ...
    async def on_tools_update(self, async_nyra_env, tool):
        raise NotImplementedError
    ...
    async def on_data_chat_completion(
        ...
        sentences = []
        self.nyra_env.log_info(f"messages: {input_messages}")
        response = self._stream_chat(query=input_messages[0]["content"])
        async for message in response:
            # self.nyra_env.log_info(f"content: {message}")
            ...
            if message_type == "message" or message_type == "agent_message":
                if not self.conversational_id and message.get("conversation_id"):
                    ...
                    nyra_env.log_info(f"conversation_id: {self.conversational_id}")
                ...
                nyra_env.log_info(f"metadata: {metadata}")
            elif message_type == "error":
                err_message = message.get("message", {})
                nyra_env.log_error(f"error: {err_message}")
            ...
            try:
                ...
                async with session.post(url, json=payload, headers=headers) as response:
                    if response.status != 200:
                        ...
                        return
                    end_time = time.time()
                    self.nyra_env.log_info(f"connect time {end_time - start_time} s")

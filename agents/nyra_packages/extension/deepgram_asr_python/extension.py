...
class DeepgramASRExtension(AsyncExtension):
    ...
    async def on_init(self, nyra_env: AsyncTenEnv) -> None:
        nyra_env.log_info("DeepgramASRExtension on_init")

    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        nyra_env.log_info("on_start")
        self.loop = asyncio.get_event_loop()
        self.nyra_env = nyra_env

        self.config = await DeepgramASRConfig.create_async(nyra_env=nyra_env)
        nyra_env.log_info(f"config: {self.config}")

        if not self.config.api_key:
            nyra_env.log_error("get property api_key")
            return

        self.loop.create_task(self._start_listen())

        nyra_env.log_info("starting async_deepgram_wrapper thread")

    async def on_audio_frame(self, _: AsyncTenEnv, frame: AudioFrame) -> None:
        frame_buf = frame.get_buf()

        if not frame_buf:
            self.nyra_env.log_warn("send_frame: empty pcm_frame detected.")
            return

        if not self.connected:
            self.nyra_env.log_debug("send_frame: deepgram not connected.")
            return
    ...
    async def on_cmd(self, nyra_env: AsyncTenEnv, cmd: Cmd) -> None:
        ...
        nyra_env.log_info(f"on_cmd json: {cmd_json}")
    ...
    async def _start_listen(self) -> None:
        self.nyra_env.log_info("start and listen deepgram")
        ...
        async def on_open(_, event):
            self.nyra_env.log_info(f"deepgram event callback on_open: {event}")
            self.connected = True

        async def on_close(_, event):
            self.nyra_env.log_info(f"deepgram event callback on_close: {event}")
            self.connected = False
            if not self.stopped:
        ...
        async def on_error(_, error):
            self.nyra_env.log_error(f"deepgram event callback on_error: {error}")
        ...
        self.nyra_env.log_info(f"deepgram options: {options}")
        # connect to websocket
        result = await self.client.start(options)
        if not result:
            self.nyra_env.log_error("failed to connect to deepgram")
            await asyncio.sleep(0.2)
            self.loop.create_task(self._start_listen())
        else:
            self.nyra_env.log_info("successfully connected to deepgram")
    ...
    async def _send_text(self, text: str, is_final: bool, stream_id: str) -> None:
        ...
        asyncio.create_task(self.nyra_env.send_data(stable_data))

...
from nyra_ai_base.chat_memory import ChatMemory
...
class GeminiRealtimeExtension(AsyncLLMBaseExtension):
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        ...
        self.config = await GeminiRealtimeConfig.create_async(nyra_env=nyra_env)
        ...
        try:
            ...
            self.loop.create_task(self._loop(nyra_env))
            self.loop.create_task(self._on_video(nyra_env))
    ...
    async def _loop(self, nyra_env: AsyncTenEnv) -> None:
        while not self.stopped:
            await asyncio.sleep(1)
            try:
                config: LiveConnectConfig = self._get_session_config()
                nyra_env.log_info("Start listen")
                async with self.client.aio.live.connect(
                    model=self.config.model, config=config
                ) as session:
                    nyra_env.log_info("Connected")
                    ...
                    while True:
                        try:
                            async for response in session.receive():
                                ...
                                # nyra_env.log_info(f"Received response")
                                try:
                                    if response.server_content:
                                        if response.server_content.interrupted:
                                            nyra_env.log_info("Interrupted")
                                            await self._flush()
                                            continue
                                        elif (
                                            ...
                                            nyra_env.log_info("Turn complete")
                                    elif response.setup_complete:
                                        nyra_env.log_info("Setup complete")
                                    ...
                                    traceback.print_exc()
                                    nyra_env.log_error("Failed to handle response")

                            await self._flush()
                            nyra_env.log_info("Finish listen")
                        except websockets.exceptions.ConnectionClosedOK:
                            nyra_env.log_info("Connection closed")
                            break
            except Exception as e:
                self.nyra_env.log_error(f"Failed to handle loop {e}")

    async def send_audio_out(
        self, nyra_env: AsyncTenEnv, audio_data: bytes, **args: TTSPcmOptions
    ) -> None:
        ...
        try:
            ...
            if combined_data:
                ...
                buff[:] = combined_data
                f.unlock_buf(buff)
                await nyra_env.send_audio_frame(f)
        except Exception:
            pass
            # nyra_env.log_error(f"error send audio frame, {traceback.format_exc()}")
    ...
    async def on_audio_frame(
        self, nyra_env: AsyncTenEnv, audio_frame: AudioFrame
    ) -> None:
        await super().on_audio_frame(nyra_env, audio_frame)
        try:
            ...
            traceback.print_exc()
            self.nyra_env.log_error(f"on audio frame failed {e}")

    async def on_cmd(self, nyra_env: AsyncTenEnv, cmd: Cmd) -> None:
        cmd_name = cmd.get_name()
        nyra_env.log_debug(f"on_cmd name {cmd_name}")
    ...
    async def on_video_frame(self, async_nyra_env, video_frame):
        await super().on_video_frame(async_nyra_env, video_frame)
    ...
    async def _on_video(self, _: AsyncTenEnv):
        while True:
            ...
            try:
                if self.connected:
                    # nyra_env.log_info(f"send image")
                    await self.session.send(media_chunks)
            except Exception as e:
                self.nyra_env.log_error(f"Failed to send image {e}")
    ...
    async def _on_audio(self, buff: bytearray):
        ...
        if self.connected and len(self.buff) >= self.audio_len_threshold:
            ...
            try:
                ...
                # pass
                self.nyra_env.log_error(f"Failed to send audio {e}")
    ...
    async def on_tools_update(
        self, nyra_env: AsyncTenEnv, tool: LLMToolMetadata
    ) -> None:
        """Called when a new tool is registered. Implement this method to process the new tool."""
        nyra_env.log_info(f"on tools update {tool}")
    ...
    async def _handle_tool_call(self, func_calls: list[FunctionCall]) -> None:
        ...
        try:
            self.nyra_env.log_info(f"send tool response {function_responses}")
            await self.session.send(
                LiveClientToolResponse(function_responses=function_responses)
            )
        except Exception as e:
            self.nyra_env.log_error(f"Failed to send tool response {e}")

...
class MessageCollectorRTMExtension(AsyncExtension):
    ...
    async def on_init(self, nyra_env: AsyncTenEnv) -> None:
        nyra_env.log_info("MessageCollectorRTMExtension on_init")

    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        nyra_env.log_info("MessageCollectorRTMExtension on_start")
    ...
    async def on_data(self, nyra_env: AsyncTenEnv, data: Data) -> None:
        ...
        if data_name == "text_data":
            ...
            nyra_env.log_warn(f"unsupported data {data_name}")
    ...
    async def on_rtm_message_event(self, data: Data) -> None:
        self.nyra_env.log_debug("on_data rtm_message_event")
        try:
            ...
            self.nyra_env.log_error(f"Failed to handle on_rtm_message_event data: {e}")
    ...
    async def _process_queue(self):
        ...
        while not self.stopped:
            try:
                ...
                self.nyra_env.log_error(f"Failed to process queue: {e}")

    async def _handle_text_data(self, data: dict):
        try:
            self.nyra_env.log_debug(f"Handling text data: {data}")
            json_bytes = json.dumps(data).encode("utf-8")
            cmd = Cmd.create("publish")
            cmd.set_property_buf("message", json_bytes)
            [cmd_result, _] = await self.nyra_env.send_cmd(cmd)
            self.nyra_env.log_info(f"send_cmd result {cmd_result.to_json()}")
        except Exception as e:
            self.nyra_env.log_error(f"Failed to handle text data: {e}")
    ...
    async def _handle_user_state(self, data: dict):
        try:
            ...
            self.nyra_env.log_error(f"Failed to handle user state: {e}")

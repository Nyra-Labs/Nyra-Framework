...
class HTTPServerExtension(Extension):
    def __init__(self, name: str):
        super().__init__(name)
        self.lisnyra_addr = "127.0.0.1"
        self.lisnyra_port = 8888
        self.cmd_white_list = None
        self.server = None
        self.thread = None

    def on_start(self, ten: TenEnv):
        self.lisnyra_addr = ten.get_property_string("lisnyra_addr")
        self.lisnyra_port = ten.get_property_int("lisnyra_port")
        ...
        self.server = HTTPServer(
            (self.lisnyra_addr, self.lisnyra_port), partial(HTTPHandler, ten)
        )

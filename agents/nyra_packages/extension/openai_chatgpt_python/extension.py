...
from nyra_ai_base.helper import (
    AsyncEventEmitter,
    get_property_bool,
    get_property_string,
)
...
class OpenAIChatGPTExtension(AsyncLLMBaseExtension):
    ...
    async def on_init(self, async_nyra_env: AsyncTenEnv) -> None:
        async_nyra_env.log_info("on_init")
        await super().on_init(async_nyra_env)

    async def on_start(self, async_nyra_env: AsyncTenEnv) -> None:
        async_nyra_env.log_info("on_start")
        await super().on_start(async_nyra_env)

        self.config = await OpenAIChatGPTConfig.create_async(nyra_env=async_nyra_env)

        # Mandatory properties
        if not self.config.api_key:
            async_nyra_env.log_info("API key is missing, exiting on_start")
            return

        # Create instance
        try:
            self.client = OpenAIChatGPT(async_nyra_env, self.config)
            async_nyra_env.log_info(
                f"initialized with max_tokens: {self.config.max_tokens}, model: {self.config.model}, vendor: {self.config.vendor}"
            )
        except Exception as err:
            async_nyra_env.log_info(f"Failed to initialize OpenAIChatGPT: {err}")

    async def on_stop(self, async_nyra_env: AsyncTenEnv) -> None:
        async_nyra_env.log_info("on_stop")
        await super().on_stop(async_nyra_env)

    async def on_deinit(self, async_nyra_env: AsyncTenEnv) -> None:
        async_nyra_env.log_info("on_deinit")
        await super().on_deinit(async_nyra_env)

    async def on_cmd(self, async_nyra_env: AsyncTenEnv, cmd: Cmd) -> None:
        cmd_name = cmd.get_name()
        async_nyra_env.log_info(f"on_cmd name: {cmd_name}")

        if cmd_name == CMD_IN_FLUSH:
            await self.flush_input_items(async_nyra_env)
            await async_nyra_env.send_cmd(Cmd.create(CMD_OUT_FLUSH))
            async_nyra_env.log_info("on_cmd sent flush")
            status_code, detail = StatusCode.OK, "success"
            cmd_result = CmdResult.create(status_code)
            cmd_result.set_property_string("detail", detail)
            await async_nyra_env.return_result(cmd_result, cmd)
        elif cmd_name == CMD_IN_ON_USER_JOINED:
            self.users_count += 1
            # Send greeting when first user joined
            if self.config.greeting and self.users_count == 1:
                self.send_text_output(async_nyra_env, self.config.greeting, True)
            ...
            await super().on_cmd(async_nyra_env, cmd)

    async def on_data(self, async_nyra_env: AsyncTenEnv, data: Data) -> None:
        data_name = data.get_name()
        async_nyra_env.log_debug("on_data name {}".format(data_name))
        ...
        if not is_final:
            async_nyra_env.log_debug("ignore non-final input")
            return
        if not input_text:
            async_nyra_env.log_warn("ignore empty text")
            return

        async_nyra_env.log_info(f"OnData input text: [{input_text}]")
    ...
    async def on_tools_update(
        self, async_nyra_env: AsyncTenEnv, tool: LLMToolMetadata
    ) -> None:
        return await super().on_tools_update(async_nyra_env, tool)

    async def on_call_chat_completion(
        self, async_nyra_env: AsyncTenEnv, **kargs: LLMCallCompletionArgs
    ) -> any:
        kmessages: LLMChatCompletionUserMessageParam = kargs.get("messages", [])

        async_nyra_env.log_info(f"on_call_chat_completion: {kmessages}")
        response = await self.client.get_chat_completions(kmessages, None)
        return response.to_json()

    async def on_data_chat_completion(
        self, async_nyra_env: AsyncTenEnv, **kargs: LLMDataCompletionArgs
    ) -> None:
        ...
        if not kmessage:
            async_nyra_env.log_error("No message in data")
            return
        ...
        memory = self.memory
        try:
            async_nyra_env.log_info(f"for input text: [{message}] memory: {memory}")
            tools = None
            ...
            if not no_tool and len(self.available_tools) > 0:
                ...
                for tool in self.available_tools:
                    ...
                    async_nyra_env.log_info(f"tool: {tool}")
            ...
            async def handle_tool_call(tool_call):
                ...
                async_nyra_env.log_info(f"tool_call: {tool_call}")
                for tool in self.available_tools:
                    if tool_call["function"]["name"] == tool.name:
                        ...
                        [result, _] = await async_nyra_env.send_cmd(cmd)
                        if result.get_status_code() == StatusCode.OK:
                            tool_result: LLMToolResult = json.loads(
                                result.get_property_to_json(CMD_PROPERTY_RESULT)
                            )

                            async_nyra_env.log_info(f"tool_result: {tool_result}")
                            # self.memory_cache = []
                            ...
                            async_nyra_env.log_error("Tool call failed")
            ...
            async def handle_content_update(content: str):
                ...
                for s in sentences:
                    self.send_text_output(async_nyra_env, s, False)
            ...
            async_nyra_env.log_info(f"Task cancelled: {message}")
        except Exception:
            async_nyra_env.log_error(
                f"Error in chat_completion: {traceback.format_exc()} for input text: {message}"
            )
        finally:
            self.send_text_output(async_nyra_env, "", True)

...
class OpenAIChatGPT:
    ...
    def __init__(self, nyra_env: AsyncTenEnv, config: OpenAIChatGPTConfig):
        self.config = config
        nyra_env.log_info(f"OpenAIChatGPT initialized with config: {config.api_key}")
        ...
        if config.proxy_url:
            ...
            nyra_env.log_info(f"Setting proxies: {proxies}")

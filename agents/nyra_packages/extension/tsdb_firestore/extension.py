...
class TSDBFirestoreExtension(Extension):
    ...
    async def __thread_routine(self, nyra_env: TenEnv):
        nyra_env.log_info("__thread_routine start")
    ...
    def on_init(self, nyra_env: TenEnv) -> None:
        nyra_env.log_info("TSDBFirestoreExtension on_init")
        nyra_env.on_init_done()

    def on_start(self, nyra_env: TenEnv) -> None:
        nyra_env.log_info("TSDBFirestoreExtension on_start")

        try:
            self.credentials = nyra_env.get_property_to_json(PROPERTY_CREDENTIALS)
        ...
        try:
            self.channel_name = nyra_env.get_property_string(PROPERTY_CHANNEL_NAME)
        except Exception as err:
            nyra_env.log_error(
        ...
        try:
            self.collection_name = nyra_env.get_property_string(PROPERTY_COLLECTION_NAME)
        except Exception as err:
            nyra_env.log_error(
        ...
        self.thread = threading.Thread(target=self.async_handle, args=[nyra_env])
        self.thread.start()

        # start the loop to handle cmd in
        self.cmd_thread = threading.Thread(
            target=asyncio.run, args=(self.__thread_routine(nyra_env),)
        )
        self.cmd_thread.start()

    def async_handle(self, nyra_env: TenEnv) -> None:
        while not self.stopped:
            try:
                value = self.queue.get()
                if value is None:
                    nyra_env.log_info("exit handle loop")
                    break
                ...
                nyra_env.log_error("Failed to store chat contents")

    def on_stop(self, nyra_env: TenEnv) -> None:
        nyra_env.log_info("TSDBFirestoreExtension on_stop")
    ...
    def on_deinit(self, nyra_env: TenEnv) -> None:
        nyra_env.log_info("TSDBFirestoreExtension on_deinit")
        nyra_env.on_deinit_done()

    def on_cmd(self, nyra_env: TenEnv, cmd: Cmd) -> None:
        try:
            cmd_name = cmd.get_name()
            nyra_env.log_info(f"on_cmd name {cmd_name}")
            if cmd_name == RETRIEVE_CMD:
                asyncio.run_coroutine_threadsafe(self.retrieve(nyra_env, cmd), self.loop)
            else:
                nyra_env.log_info(f"unknown cmd name {cmd_name}")
                cmd_result = CmdResult.create(StatusCode.ERROR)
                nyra_env.return_result(cmd_result, cmd)
        except Exception:
            nyra_env.return_result(CmdResult.create(StatusCode.ERROR), cmd)

    async def retrieve(self, nyra_env: TenEnv, cmd: Cmd):
        try:
            ...
            if DOC_CONTENTS_PATH in doc_dict:
                ...
                nyra_env.log_info(f"after retrieve {contents}")
                ret = CmdResult.create(StatusCode.OK)
                ret.set_property_string(
                    CMD_OUT_PROPERTY_RESPONSE, json.dumps(order_by_ts(contents))
                )
                nyra_env.return_result(ret, cmd)
            else:
                nyra_env.log_info(f"no contents for the channel {self.channel_name} yet")
                nyra_env.return_result(CmdResult.create(StatusCode.ERROR), cmd)
        except Exception:
            nyra_env.log_error(
    ...
    def on_data(self, nyra_env: TenEnv, data: Data) -> None:
        nyra_env.log_info("TSDBFirestoreExtension on_data")
        ...
        try:
            ...
            if not role:
                nyra_env.log_warn("ignore empty role")
                return
        except Exception as err:
            nyra_env.log_info(

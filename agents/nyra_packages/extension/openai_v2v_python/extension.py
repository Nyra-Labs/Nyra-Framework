...
from nyra_ai_base.const import CMD_PROPERTY_RESULT, CMD_TOOL_CALL
from nyra_ai_base import AsyncLLMBaseExtension
from dataclasses import dataclass
from nyra_ai_base.config import BaseConfig
from nyra_ai_base.chat_memory import (
    ChatMemory,
    EVENT_MEMORY_EXPIRED,
    EVENT_MEMORY_APPENDED,
)
from nyra_ai_base.usage import (
    LLMUsage,
    LLMCompletionTokensDetails,
    LLMPromptTokensDetails,
)
from nyra_ai_base.types import (
    LLMToolMetadata,
    LLMToolResult,
...
class OpenAIRealtimeExtension(AsyncLLMBaseExtension):
    ...
    def __init__(self, name: str):
        super().__init__(name)
        self.nyra_env: AsyncTenEnv = None
        self.conn = None
        self.session = None
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        ...
        self.config = await OpenAIRealtimeConfig.create_async(nyra_env=nyra_env)
        nyra_env.log_info(f"config: {self.config}")

        if not self.config.api_key:
            nyra_env.log_error("api_key is required")
            return
        ...
        try:
            ...
            if self.config.enable_storage:
                [result, _] = await nyra_env.send_cmd(Cmd.create("retrieve"))
                if result.get_status_code() == StatusCode.OK:
                    try:
                        history = json.loads(result.get_property_string("response"))
                        for i in history:
                            self.memory.put(i)
                        nyra_env.log_info(f"on retrieve context {history}")
                    except Exception as e:
                        nyra_env.log_error(f"Failed to handle retrieve result {e}")
                else:
                    nyra_env.log_warn("Failed to retrieve content")
            ...
            nyra_env.log_info("Finish init client")

            self.loop.create_task(self._loop())
        except Exception as e:
            traceback.print_exc()
            self.nyra_env.log_error(f"Failed to init client {e}")

    async def on_stop(self, nyra_env: AsyncTenEnv) -> None:
        await super().on_stop(nyra_env)
        nyra_env.log_info("on_stop")
    ...
    async def on_audio_frame(self, _: AsyncTenEnv, audio_frame: AudioFrame) -> None:
        try:
            ...
            traceback.print_exc()
            self.nyra_env.log_error(f"OpenAIV2VExtension on audio frame failed {e}")
    ...
    async def on_cmd(self, nyra_env: AsyncTenEnv, cmd: Cmd) -> None:
        ...
        if cmd_name == CMD_IN_FLUSH:
            ...
            await self._flush()
            await nyra_env.send_cmd(Cmd.create(CMD_OUT_FLUSH))
            nyra_env.log_info("on flush")
    ...
    async def _loop(self):
        ...
        try:
            ...
            self.nyra_env.log_info("Client loop started")
            async for message in self.conn.listen():
                try:
                    # self.nyra_env.log_info(f"Received message: {message.type}")
                    match message:
                        case SessionCreated():
                            self.nyra_env.log_info(
                            ...
                            self.nyra_env.log_info(f"Finish send history {history}")
                            self.memory.clear()
                        ...
                        case ItemInputAudioTranscriptionFailed():
                            self.nyra_env.log_warn(
                                f"On request transcript failed {message.item_id} {message.error}"
                            )
                        case ItemCreated():
                            self.nyra_env.log_info(f"On item created {message.item}")
                        case ResponseCreated():
                            response_id = message.response.id
                            self.nyra_env.log_info(f"On response created {response_id}")
                        case ResponseDone():
                        ...
                        case ResponseOutputItemDone():
                            self.nyra_env.log_info(f"Output item done {message.item}")
                        ...
                        case ResponseFunctionCallArgumentsDone():
                            ...
                            self.nyra_env.log_info(f"need to call func {name}")
                            self.loop.create_task(
                                self._handle_tool_call(tool_call_id, name, arguments)
                            )
                        case ErrorMessage():
                            self.nyra_env.log_error(
                                f"Error message received: {message.error}"
                            )
                        case _:
                            self.nyra_env.log_debug(f"Not handled message {message}")
                except Exception as e:
                    traceback.print_exc()
                    self.nyra_env.log_error(f"Error processing message: {message} {e}")

            self.nyra_env.log_info("Client loop finished")
        except Exception as e:
            traceback.print_exc()
            self.nyra_env.log_error(f"Failed to handle loop {e}")
        ...
        if not self.stopped:
            ...
            self.nyra_env.log_info("Reconnect")

            self.conn = RealtimeApiConnection(
                nyra_env=self.nyra_env,
                base_uri=self.config.base_uri,
                path=self.config.path,
                api_key=self.config.api_key,
                model=self.config.model,
    ...
    async def _on_memory_appended(self, message: dict) -> None:
        ...
        try:
            ...
            asyncio.create_task(self.nyra_env.send_data(d))
        except Exception as e:
            self.nyra_env.log_error(f"Error send append_context data {message} {e}")
    ...
    async def _update_session(self) -> None:
        ...
        self.nyra_env.log_info(f"update session {prompt} {tools}")
        su = SessionUpdate(
    ...
    async def on_tools_update(self, _: AsyncTenEnv, tool: LLMToolMetadata) -> None:
        ...
        self.nyra_env.log_info(f"on tools update {tool}")
        # await self._update_session()
    ...
    async def _on_audio_delta(self, delta: bytes) -> None:
        ...
        buff[:] = audio_data
        f.unlock_buf(buff)
        await self.nyra_env.send_audio_frame(f)
    ...
    def _send_transcript(self, content: str, role: Role, is_final: bool) -> None:
        ...
        def send_data(
            ...
            try:
                ...
                asyncio.create_task(nyra_env.send_data(d))
            except Exception as e:
                nyra_env.log_error(
        ...
        try:
            if role == Role.Assistant and not is_final:
                ...
                for s in sentences:
                    send_data(self.nyra_env, s, stream_id, role, is_final)
            else:
                send_data(self.nyra_env, content, stream_id, role, is_final)
        except Exception as e:
            self.nyra_env.log_error(
    ...
    async def _handle_tool_call(
        ...
        self.nyra_env.log_info(f"_handle_tool_call {tool_call_id} {name} {arguments}")
        cmd: Cmd = Cmd.create(CMD_TOOL_CALL)
        cmd.set_property_string("name", name)
        cmd.set_property_from_json("arguments", arguments)
        [result, _] = await self.nyra_env.send_cmd(cmd)
        ...
        if result.get_status_code() == StatusCode.OK:
            ...
            self.nyra_env.log_info(f"tool_result: {tool_call_id} {tool_result}")
        else:
            self.nyra_env.log_error("Tool call failed")

        await self.conn.send_request(tool_response)
        await self.conn.send_request(ResponseCreate())
        self.nyra_env.log_info(f"_remote_tool_call finish {name} {arguments}")
    ...
    async def _greeting(self) -> None:
        if self.connected and self.users_count == 1:
            ...
            self.nyra_env.log_info(f"send greeting {text}")
            await self.conn.send_request(
                ItemCreate(
                    item=UserMessageItemParam(
    ...
    async def _flush(self) -> None:
        try:
            c = Cmd.create("flush")
            await self.nyra_env.send_cmd(c)
        except Exception:
            self.nyra_env.log_error("Error flush")
    ...
    async def _update_usage(self, usage: dict) -> None:
        ...
        self.nyra_env.log_info(f"total usage: {self.total_usage}")
        ...
        asyncio.create_task(self.nyra_env.send_data(data))

    async def on_call_chat_completion(self, async_nyra_env, **kargs):
        raise NotImplementedError

    async def on_data_chat_completion(self, async_nyra_env, **kargs):
        raise NotImplementedError

...
class RealtimeApiConnection:
    ...
    async def send_request(self, message: ClientToServerMessage):
        ...
        if self.verbose:
            self.nyra_env.log_info(f"-> {smart_str(message_str)}")
        await self.websocket.send_str(message_str)

    async def listen(self) -> AsyncGenerator[ServerToClientMessage, None]:
        assert self.websocket is not None
        if self.verbose:
            self.nyra_env.log_info("Listening for realtimeapi messages")
        try:
            async for msg in self.websocket:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    if self.verbose:
                        self.nyra_env.log_info(f"<- {smart_str(msg.data)}")
                    yield self.handle_server_message(msg.data)
                elif msg.type == aiohttp.WSMsgType.ERROR:
                    self.nyra_env.log_error("Error during receive: %s", self.websocket.exception())
                    break
        except asyncio.CancelledError:
            self.nyra_env.log_info("Receive messages task cancelled")

    def handle_server_message(self, message: str) -> ServerToClientMessage:
        try:
            return parse_server_message(message)
        except Exception as e:
            self.nyra_env.log_info(f"Error handling message {e}")

...
class MinimaxV2VConfig:
    ...
    async def read_from_property(self, nyra_env: AsyncTenEnv):
        for field in fields(self):
            ...
            # if not nyra_env.is_property_exist(field.name):
            #     continue
            try:
                match field.type:
                    case builtins.str:
                        val = await nyra_env.get_property_string(field.name)
                        if val:
                            setattr(self, field.name, val)
                            nyra_env.log_info(f"{field.name}={val}")
                    case builtins.int:
                        val = await nyra_env.get_property_int(field.name)
                        setattr(self, field.name, val)
                        nyra_env.log_info(f"{field.name}={val}")
                    case builtins.bool:
                        val = await nyra_env.get_property_bool(field.name)
                        setattr(self, field.name, val)
                        nyra_env.log_info(f"{field.name}={val}")
                    case _:
                        pass
            except Exception as e:
                nyra_env.log_warn(f"get property for {field.name} failed, err {e}")
...
class MinimaxV2VExtension(AsyncExtension):
    ...
    async def on_init(self, nyra_env: AsyncTenEnv) -> None:
        await self.config.read_from_property(nyra_env=nyra_env)
    ...
    async def on_start(self, nyra_env: AsyncTenEnv) -> None:
        self.process_input_task = asyncio.create_task(
            self._process_input(nyra_env=nyra_env, queue=self.queue), name="process_input"
        )

    async def on_stop(self, nyra_env: AsyncTenEnv) -> None:

        await self._flush(nyra_env=nyra_env)
    ...
    async def on_cmd(self, nyra_env: AsyncTenEnv, cmd: Cmd) -> None:
        try:
            ...
            match cmd_name:
                case "flush":
                    await self._flush(nyra_env=nyra_env)
                    await nyra_env.send_cmd(Cmd.create("flush"))
                    nyra_env.log_debug("flush done")
                case _:
                    pass
            await nyra_env.return_result(CmdResult.create(StatusCode.OK), cmd)
        except asyncio.CancelledError:
            nyra_env.log_warn(f"cmd {cmd_name} cancelled")
            await nyra_env.return_result(CmdResult.create(StatusCode.ERROR), cmd)
            raise
        except Exception as e:
            nyra_env.log_warn(f"cmd {cmd_name} failed, err {e}")
        finally:
            pass
    ...
    async def on_audio_frame(
        ...
        try:
            ...
            nyra_env.log_debug(f"on audio frame {len(frame_buf)} {stream_id}")
            ...
            # nyra_env.log_debug(f"on audio frame {len(frame_buf)} {stream_id} put done")
        except asyncio.CancelledError:
            nyra_env.log_warn("on audio frame cancelled")
            raise
        except Exception as e:
            nyra_env.log_error(f"on audio frame failed, err {e}")

    async def on_video_frame(
        self, nyra_env: AsyncTenEnv, video_frame: VideoFrame
    ) -> None:
        pass

    async def _process_input(self, nyra_env: AsyncTenEnv, queue: asyncio.Queue):
        nyra_env.log_info("process_input started")
        ...
        while True:
            ...
            (ts, frame_buf) = item
            nyra_env.log_debug(f"start process task {ts} {len(frame_buf)}")
            ...
            try:
                ...
                nyra_env.log_warn("task cancelled")
            except Exception as e:
                nyra_env.log_warn(f"task failed, err {e}")
            finally:
                queue.task_done()

        nyra_env.log_info("process_input exit")
    ...
    async def _complete_with_history(self, ts: datetime, buff: bytearray):
        ...
        nyra_env.log_debug(f"messages without audio: [{messages}]")
        messages.append(
        ...
        try:
            ...
            async with self.client.stream(
                ...
                async for line in response.aiter_lines():
                    # nyra_env.log_info(f"-> line {line}")
                    # if self._need_interrupt(ts):
                    #     nyra_env.log_warn(f"trace-id: {trace_id}, interrupted")
                    #     if self.transcript:
                    ...
                    if not line.startswith("data:"):
                        nyra_env.log_debug(f"ignore line {len(line)}")
                        continue
                    i += 1
                        ...
                        if delta.get("role") == "assistant":
                            ...
                            if (
                                ...
                                await self._send_audio_frame(
                                    nyra_env=nyra_env, audio_data=buff
                                )

                            # tool calls
                            if delta.get("tool_calls"):
                                nyra_env.log_warn(f"ignore tool call {delta}")
                                # TODO: add tool calls
                                continue
            ...
            nyra_env.log_warn("http timeout")
        except httpx.HTTPStatusError as e:
            nyra_env.log_warn(f"http status error: {e}")
        except httpx.RequestError as e:
            nyra_env.log_warn(f"http request error: {e}")
        finally:
            nyra_env.log_info(
    ...
    async def _send_audio_frame(
        self, nyra_env: AsyncTenEnv, audio_data: bytearray
    ) -> None:
        try:
            ...
            nyra_env.log_error(f"send audio frame failed, err {e}")
    ...
    async def _flush(self, nyra_env: AsyncTenEnv) -> None:
        # clear queue
        while not self.queue.empty():
            try:
                ...
                nyra_env.log_warn(f"flush queue error {e}")

...
class PollyTTS:
    def __init__(self, config: PollyTTSConfig, nyra_env: AsyncTenEnv) -> None:
        """
        :param config: A PollyConfig
        """
        nyra_env.log_info("startinit polly tts")
        self.config = config
    ...
    def _synthesize(self, text, nyra_env: AsyncTenEnv):
        """
        ...
        try:
            ...
            if self.config.include_visemes:
                ...
                nyra_env.log_debug("Got %s visemes.", len(visemes))
        except ClientError:
            nyra_env.log_error("Couldn't get audio stream.")
            raise
        else:
            return audio_stream, visemes

    async def text_to_speech_stream(
        self, nyra_env: AsyncTenEnv, text: str
    ) -> AsyncIterator[bytes]:
        inputText = text
        if len(inputText) == 0:
            nyra_env.log_warning("async_polly_handler: empty input detected.")
        try:
            audio_stream, _ = self._synthesize(inputText, nyra_env)
            with closing(audio_stream) as stream:
                for chunk in stream.iter_chunks(chunk_size=self.frame_size):
                    yield chunk
        except Exception:
            nyra_env.log_error(traceback.format_exc())

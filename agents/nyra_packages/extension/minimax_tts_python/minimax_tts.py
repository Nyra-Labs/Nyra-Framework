...
class MinimaxTTS:
    ...
    async def get(self, nyra_env: AsyncTenEnv, text: str) -> AsyncIterator[bytes]:
        payload = json.dumps(
            {
        ...
        nyra_env.log_info(f"Start request, url: {self.config.url}, text: {text}")
        ttfb = None
            ...
            try:
                async with session.post(url, headers=headers, data=payload) as response:
                    ...
                    try:
                        ...
                        nyra_env.log_warn("get response, no Trace-Id")
                    try:
                        alb_receive_time = response.headers.get("alb_receive_time")
                    except Exception:
                        nyra_env.log_warn("get response, no alb_receive_time")
                        ...
                        while b"\n" in buffer:
                            ...
                            if line.startswith(b"data:"):
                                try:
                                    ...
                                    nyra_env.log_warn(f"Error decoding line: {e}")
                                    continue
                        if not ttfb:
                            ttfb = self._duration_in_ms_since(start_time)
                            nyra_env.log_info(f"trace-id: {trace_id}, ttfb {ttfb}ms")
            except aiohttp.ClientError as e:
                nyra_env.log_error(f"Client error occurred: {e}")
            except asyncio.TimeoutError:
                nyra_env.log_error("Request timed out")
            finally:
                nyra_env.log_info(

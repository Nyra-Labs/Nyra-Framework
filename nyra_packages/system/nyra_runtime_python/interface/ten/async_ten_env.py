...
from .nyra_env_base import TenEnvBase
...
class AsyncTenEnv(TenEnvBase):
    ...
    def __init__(
        self, nyra_env: TenEnv, loop: AbstractEventLoop, thread: threading.Thread
    ) -> None:
        super().__init__(nyra_env._internal)

        self._nyra_loop = loop
        self._nyra_thread = thread
        nyra_env._set_release_handler(lambda: self._on_release())

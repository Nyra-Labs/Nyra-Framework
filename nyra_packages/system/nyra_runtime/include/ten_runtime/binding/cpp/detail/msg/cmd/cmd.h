...
#include "nyra_runtime/msg/cmd/cmd.h"
...
class cmd_t : public msg_t {
 ...
 public:
  static std::unique_ptr<cmd_t> create(const char *name,
                                       error_t *err = nullptr) {
    nyra_shared_ptr_t *c_cmd =
        nyra_cmd_create(name, err != nullptr ? err->get_c_error() : nullptr);

    return std::make_unique<cmd_t>(c_cmd, ctor_passkey_t());
  }

  explicit cmd_t(nyra_shared_ptr_t *cmd, ctor_passkey_t /*unused*/)
      : msg_t(cmd) {}
 ...
 protected:
  ...
  // first.
  explicit cmd_t(nyra_shared_ptr_t *cmd) : msg_t(cmd) {}
  // @}
 ...
 private:
  ...
  void clone_internal(const cmd_t &cmd) noexcept {
    if (cmd.c_msg != nullptr) {
      c_msg = nyra_msg_clone(cmd.c_msg, nullptr);
    } else {
      c_msg = nullptr;
    }
  }

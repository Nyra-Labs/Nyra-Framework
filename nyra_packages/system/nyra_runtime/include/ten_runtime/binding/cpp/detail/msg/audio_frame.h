...
#include "nyra_runtime/binding/cpp/detail/msg/msg.h"
#include "nyra_runtime/msg/audio_frame/audio_frame.h"
...
class audio_frame_t : public msg_t {
 ...
 public:
  ...
  int64_t get_timestamp(error_t *err = nullptr) const {
    return nyra_audio_frame_get_timestamp(c_msg);
  }
  bool set_timestamp(int64_t timestamp, error_t *err = nullptr) {
    return nyra_audio_frame_set_timestamp(c_msg, timestamp);
  }

  int32_t get_sample_rate(error_t *err = nullptr) const {
    return nyra_audio_frame_get_sample_rate(c_msg);
  }
  bool set_sample_rate(int32_t sample_rate, error_t *err = nullptr) {
    return nyra_audio_frame_set_sample_rate(c_msg, sample_rate);
  }

  uint64_t get_channel_layout(error_t *err = nullptr) const {
    return nyra_audio_frame_get_channel_layout(c_msg);
  }
  bool set_channel_layout(uint64_t channel_layout, error_t *err = nullptr) {
    return nyra_audio_frame_set_channel_layout(c_msg, channel_layout);
  }

  int32_t get_samples_per_channel(error_t *err = nullptr) const {
    return nyra_audio_frame_get_samples_per_channel(c_msg);
  }
  bool set_samples_per_channel(int32_t samples_per_channel,
                               error_t *err = nullptr) {
    return nyra_audio_frame_set_samples_per_channel(c_msg, samples_per_channel);
  }

  int32_t get_bytes_per_sample(error_t *err = nullptr) const {
    return nyra_audio_frame_get_bytes_per_sample(c_msg);
  }
  bool set_bytes_per_sample(int32_t size, error_t *err = nullptr) {
    return nyra_audio_frame_set_bytes_per_sample(c_msg, size);
  }

  int32_t get_number_of_channels(error_t *err = nullptr) const {
    return nyra_audio_frame_get_number_of_channel(c_msg);
  }
  bool set_number_of_channels(int32_t number, error_t *err = nullptr) {
    return nyra_audio_frame_set_number_of_channel(c_msg, number);
  }

  NYRA_AUDIO_FRAME_DATA_FMT get_data_fmt(error_t *err = nullptr) const {
    return nyra_audio_frame_get_data_fmt(c_msg);
  }
  bool set_data_fmt(NYRA_AUDIO_FRAME_DATA_FMT format, error_t *err = nullptr) {
    return nyra_audio_frame_set_data_fmt(c_msg, format);
  }

  int32_t get_line_size(error_t *err = nullptr) const {
    return nyra_audio_frame_get_line_size(c_msg);
  }
  bool set_line_size(int32_t line_size, error_t *err = nullptr) {
    return nyra_audio_frame_set_line_size(c_msg, line_size);
  }

  bool is_eof(error_t *err = nullptr) const {
    return nyra_audio_frame_is_eof(c_msg);
  }
  bool set_eof(bool eof, error_t *err = nullptr) {
    return nyra_audio_frame_set_eof(c_msg, eof);
  }

  bool alloc_buf(size_t size, error_t *err = nullptr) {
    return nyra_audio_frame_alloc_buf(c_msg, size) != nullptr;
  }

  buf_t lock_buf(error_t *err = nullptr) const {
    nyra_buf_t *data = nyra_audio_frame_peek_buf(c_msg);

    if (!nyra_msg_add_locked_res_buf(
  ...
  bool unlock_buf(buf_t &buf, error_t *err = nullptr) {
    ...
    if (!nyra_msg_remove_locked_res_buf(
    ...
    nyra_buf_init_with_owned_data(&buf.buf, 0);
  ...
  explicit audio_frame_t(nyra_shared_ptr_t *frame) : msg_t(frame) {}
  // @}

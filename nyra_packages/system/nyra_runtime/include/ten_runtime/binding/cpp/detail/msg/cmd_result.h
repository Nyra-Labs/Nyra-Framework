...
class cmd_result_t : public msg_t {
 ...
 public:
  static std::unique_ptr<cmd_result_t> create(NYRA_STATUS_CODE status_code,
                                              error_t *err = nullptr) {
    return std::make_unique<cmd_result_t>(status_code, ctor_passkey_t());
  }

  explicit cmd_result_t(NYRA_STATUS_CODE status_code, ctor_passkey_t /*unused*/)
      : msg_t(nyra_cmd_result_create(status_code)) {}
  explicit cmd_result_t(nyra_shared_ptr_t *cmd, ctor_passkey_t /*unused*/)
      : msg_t(cmd) {};

  ~cmd_result_t() override = default;

  NYRA_STATUS_CODE get_status_code(error_t *err = nullptr) const {
    return nyra_cmd_result_get_status_code(c_msg);
  }

  bool is_final(error_t *err = nullptr) const {
    return nyra_cmd_result_is_final(
        c_msg, err != nullptr ? err->get_c_error() : nullptr);
  }

  bool is_completed(error_t *err = nullptr) const {
    return nyra_cmd_result_is_completed(
        c_msg, err != nullptr ? err->get_c_error() : nullptr);
  }

  bool set_final(bool final, error_t *err = nullptr) {
    return nyra_cmd_result_set_final(
 ...
 private:
  friend extension_t;
  friend nyra_env_tester_t;
  friend nyra_env_t;
  friend cmd_result_internal_accessor_t;

  static std::unique_ptr<cmd_result_t> create(nyra_shared_ptr_t *cmd,
                                              error_t *err = nullptr) {

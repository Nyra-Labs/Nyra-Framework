...
class buf_t {
 public:
  buf_t() : buf{NYRA_BUF_STATIC_INIT_OWNED} {}

  explicit buf_t(size_t size) { nyra_buf_init_with_owned_data(&buf, size); }

  buf_t(uint8_t *data, size_t size) {
    nyra_buf_init_with_unowned_data(&buf, data, size);
  }

  // Copy constructor.
  buf_t(const buf_t &other) {
    nyra_buf_init_with_copying_data(&buf, other.buf.data, other.buf.size);
  };

  // Move constructor.
  buf_t(buf_t &&other) noexcept : buf_t() { nyra_buf_move(&buf, &other.buf); };

  ~buf_t() { nyra_buf_deinit(&buf); }
 ...
 private:
  ...
  explicit buf_t(const nyra_buf_t *buf) : buf{*buf} {}

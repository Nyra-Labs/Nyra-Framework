...
#include "nyra_utils/io/stream.h"
#include "nyra_utils/io/transport.h"
#include "nyra_utils/lang/cpp/io/runloop.h"
#include "nyra_utils/lang/cpp/lib/string.h"
...
class Stream : public std::enable_shared_from_this<Stream> {
 ...
 public:
  ...
  int StartRead() {
    ...
    return nyra_stream_start_read(stream_);
  }
  ...
  int StopRead() {
    ...
    return nyra_stream_stop_read(stream_);
  }
  ...
  int Send(const char *msg, uint32_t size, void *user_data) {
    ...
    int rc = nyra_stream_send(stream_, msg, size, user_data);
    return rc;
  }
  ...
  int Close(CloseCallback &&cb) {
    ...
    nyra_stream_set_close_cb(stream_, (void *)OnClose, data);
    nyra_stream_close(stream_);
    return 0;
  }

 private:
  explicit Stream(nyra_stream_t *stream) : stream_(stream) {
    if (!stream_) {
      return;
    }
 ...
 private:
  ...
  static void OnMessageRead(nyra_stream_t *stream, void *msg, int size) {
  ...
  static void OnMessageSent(nyra_stream_t *stream, int status, void *user_data) {
    if (!stream || !stream->user_data) {
      return;
    }
  ...
  static void OnMessageFree(nyra_stream_t *stream, int status, void *user_data) {
    if (!stream || !stream->user_data) {
      return;
    }
...
class Transport : public std::enable_shared_from_this<Transport> {
 public:
  ...
  static TenTransport Create() { return Create(nyra_runloop_current()); }
 ...
 public:
  ...
  int Close(CloseCallback &&cb) {
    ...
    data->self = shared_from_this();
    nyra_transport_set_close_cb(tp_, (void *)OnClose, data);
    return nyra_transport_close(tp_);
  }
  ...
  int Listen(const TenString &uri, ClientAcceptCallback &&cb) {
    ...
    return nyra_transport_listen(tp_, uri);
  }
  ...
  int Connect(const TenString &uri, ServerConnectCallback &&cb) {
    ...
    return nyra_transport_connect(
        tp_, const_cast<nyra_string_t *>((const nyra_string_t *)uri));
  }

 private:
  static TenTransport Create(::nyra_runloop_t *loop) {
    if (!loop) {
      return nullptr;
    }

    auto tp = nyra_transport_create(loop);
    if (!tp) {
      return nullptr;
    }
  ...
  static void OnClientAccepted(nyra_transport_t *transport, nyra_stream_t *stream,
                               int status) {
  ...
  static void OnServerConnected(nyra_transport_t *transport,
                                nyra_stream_t *stream, int status) {
    if (status != 0 || !transport || !transport->user_data || !stream) {
      return;
    }
  ...
  explicit Transport(::nyra_transport_t *tp) : tp_(tp) {
    if (!tp_) {
      return;
    }

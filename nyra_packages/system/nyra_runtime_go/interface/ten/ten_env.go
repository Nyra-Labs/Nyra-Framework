...
// TenEnv represents the interface for the NYRA (Run Time Environment) component.
...
func (p *tenEnv) sendCmd(cmd Cmd, handler ResultHandler) error {
	...
	cStatus := C.nyra_go_nyra_env_send_cmd(
		p.cPtr,
		cmd.getCPtr(),
		cHandle(cb),
		C.bool(false),
...
func (p *tenEnv) SendData(data Data, handler ErrorHandler) error {
	...
	err := withCGO(func() error {
		apiStatus := C.nyra_go_nyra_env_send_data(
			p.cPtr,
			data.getCPtr(),
			cHandle(cb),
		)
...
func (p *tenEnv) SendVideoFrame(
	...
	err := withCGO(func() error {
		apiStatus := C.nyra_go_nyra_env_send_video_frame(
			p.cPtr,
			videoFrame.getCPtr(),
			cHandle(cb),
		)
...
func (p *tenEnv) SendAudioFrame(
	...
	err := withCGO(func() error {
		apiStatus := C.nyra_go_nyra_env_send_audio_frame(
			p.cPtr,
			audioFrame.getCPtr(),
			cHandle(cb),
		)
...
func (p *tenEnv) OnConfigureDone() error {
	...
	C.nyra_go_nyra_env_on_configure_done(p.cPtr)
...
func (p *tenEnv) OnInitDone() error {
	...
	C.nyra_go_nyra_env_on_init_done(p.cPtr)

	return nil
}

func (p *tenEnv) OnStartDone() error {
	C.nyra_go_nyra_env_on_start_done(p.cPtr)

	return nil
}

func (p *tenEnv) OnStopDone() error {
	C.nyra_go_nyra_env_on_stop_done(p.cPtr)

	return nil
}

func (p *tenEnv) OnDeinitDone() error {
	C.nyra_go_nyra_env_on_deinit_done(p.cPtr)
...
func (p *tenEnv) OnCreateInstanceDone(instance any, context uintptr) error {
	...
	case *extension:
		C.nyra_go_nyra_env_on_create_instance_done(p.cPtr, instance.cPtr, C.uintptr_t(context))
	default:
...
func (p *tenEnv) String() string {
	cString := C.nyra_go_nyra_env_debug_info(p.cPtr)

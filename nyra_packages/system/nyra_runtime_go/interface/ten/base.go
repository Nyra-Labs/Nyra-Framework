...
type baseTenObject[T any] struct {
	...
	// to NYRA runtime. Ex: there's nothing can do on a cmd after someone calls
	...
	// The pointer of the C bridge object used to communicate with the NYRA
	// runtime.
	cPtr T
...
//	  C.nyra_go_msg_get_property(p.cPtr, path)
//	}
//
// From the GC's (i.e., garbage collector) perspective, the `p` is unreachable
// before the `C.nyra_go_msg_get_property` even through its field `p.cPtr` stays

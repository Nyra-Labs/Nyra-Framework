...
func (p *app) Run(runInBackground bool) {
	C.nyra_go_app_run(p.cPtr, C.bool(runInBackground))
}

func (p *app) Close() {
	C.nyra_go_app_close(p.cPtr)
}

func (p *app) Wait() {
	C.nyra_go_app_wait(p.cPtr)
}
...
func NewApp(
	...
	pApp.cPtr = (*C.nyra_go_app_t)(
		unsafe.Pointer(C.nyra_go_app_create(C.uintptr_t(appObjID))),
	)

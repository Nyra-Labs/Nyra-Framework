...
from libnyra_runtime_python import _register_addon_as_extension
...
class _AddonManager:
    ...
    # NYRA runtime. This avoids using `setattr` on the module, which may not be
    ...
    def load_all_addons(cls):
        ...
        for module in glob(os.path.join(base_dir, "nyra_packages/extension/*")):
            if os.path.isdir(module):
                ...
                if module_name in extension_names:
                    cls._load_module(
                        module_full_name=(
                            f"nyra_packages.extension.{module_name}"
                        ),
